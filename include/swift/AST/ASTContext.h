...
public:
  ...
  /// In case module aliasing is used, only the alias should appear in source files. For example, if
  /// '-module-alias Foo=X' was passed, only the references to 'Foo' should be allowed, not 'X'.
  ///
  /// This checks if the real name is referenced in source files and returns false if so.
